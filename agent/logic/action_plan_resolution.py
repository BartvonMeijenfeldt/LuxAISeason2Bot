...
from logic.goal import GoalCollection, Goal
...
class Solution:
    ...
    unit_action_plans: dict[Unit, ActionPlan]
    value: float

    @property
    def joint_action_plan(self) -> dict[Unit, ActionPlan]:
        return {unit: action_plan for unit, action_plan in self.unit_action_plans.items()}
...
class ActionPlanResolver:
    def __init__(self, unit_goal_collections: dict[Unit, GoalCollection], game_state: GameState) -> None:
        self.solutions = PriorityQueue()
        self.game_state = game_state
        self.unit_goal_collections = unit_goal_collections
        self._add_root_to_queue()

    def resolve(self) -> tuple[dict[Unit, Goal], dict[Unit, ActionPlan]]:
    ...
    def _get_best_solution(self) -> Solution:
        for i in count():
            ...
            if power_collision:
                for unit in power_collision.units:
    ...
    def _init_root(self) -> Solution:
        unit_constraints = {unit: Constraints() for unit in self.unit_goal_collections.keys()}
        unit_goals = resolve_goal_conflicts(self.unit_goal_collections, self.game_state, unit_constraints)
        ...
        return Solution(unit_constraints, unit_goals, unit_action_plans, sum_value)

    def _if_valid_add_power_node(self, parent_solution: Solution, power_deficit: int, unit: Unit) -> None:

        unit_constraints = self._get_new_power_constraints(parent_solution, power_deficit, unit)
        ...
        self._optional_add_node(
            ...
            units_to_adjust_action_plan=[unit],
        )
    ...
    def _get_new_positive_tc_constraints(
        ...
        for unit, constraints in unit_constraints.items():
            ...
            if collsion_unit == unit:
                if time_coordinate.t in constraints.positive_t:
                    return None
    ...
    def _get_new_action_plan(
        self, unit: Unit, unit_goals: dict[Unit, Goal], unit_constraints: dict[Unit, Constraints]
    ) -> ActionPlan:
...
def get_power_collision(unit_action_plans: dict[Unit, ActionPlan], game_state: GameState):
    ...
    for unit, action_plan in unit_action_plans.items():
        ...
        if unit_power_requested > factories_power_available[factory]:
            ...
            return PowerCollision(units=factories_requested_by_units[factory], power_deficit=power_deficit)
